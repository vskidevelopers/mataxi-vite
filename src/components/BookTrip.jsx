...
import { SeatIcon } from "../Utils/icons";
...
export default function BookTrip() {
  ...
  const handleRadioChange = async (event) => {
    ...
    try {
      ...
      if (watchFrom && watchTo) {
        ...
        const estimatedPriceRes = await getEstimatedPrice(watchFrom, watchTo);
        if (estimatedPriceRes.data != null) {
          // if all is well then fetch vehicle
          await getVehicle(selectedRideTypeValue);
        } else {
          alert("selected route does not exist");
        }
      } else {
        alert("select  pick up point and destination first");
      }
    } catch (error) {
  ...
  const handleSeatClick = (seatId, isSeatBooked) => {
    ...
    // Update seat status based on current state
    setSeatStatus((prevStatus) => ({
      ...prevStatus,
      [seatId]: isSeatBooked
        ? "booked"
        : prevStatus[seatId] === "selected"
        ? "available"
        : "selected",
    }));
  ...
  const getEstimatedPrice = async (watchFrom, watchTo) => {
    ...
    try {
      ...
      if (getSelectedRouteResponse?.success) {
        ...
        setRoutesLoading(false);
        return { data: getSelectedRouteResponse?.data };
      } else {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-md transition-all">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      ...
                      <span className="font-bold italic">{bookingNumber}</span>.
                    </p>
                  </div>
                  ...
                  <div className="mt-4 w-full">
                    ...
                    <div className="flex justify-center items-center">
                      <Link
                        to="checkout"
                        className="rounded-md border border-transparent bg-green-400 px-4 py-2 text-sm font-medium text-[#F66E3C] hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                      >
                        Proceed to checkout{" "}
                      </Link>
                    </div>
      ...
      {/* Login Redirect popup for un authenticated users */}
          ...
          {showNoVehicleModal?.show === true ? (
                  ...
                  (_, index) => {
                    ...
                    return (
                      <div className="flex flex-col gap-1 items-center">
                        <span className="text-sm">{`seat ${index + 1}`}</span>
                        <div
                          key={seatId}
                          className={`relative rounded-lg w-8 h-8 flex items-center justify-center cursor-pointer ${seatColor}`}
            ...
            <div className="bg-[#F66E3C] text-white p-2 rounded-lg">
              {selectedPickupPoint && selectedDestination
                ...
                ? "Select pick up point and destination first"
                : `Select ${
                    selectedPickupPoint ? "destination" : "pick up point"
                  } first`}
            </div>
          )}
        </div>
        ...
        >
          {bookingLoading && (
            <div className="flex items-center">
              ...
              <span className="ml-2 text-white font-semibold">Booking...</span>
            </div>
          )}
