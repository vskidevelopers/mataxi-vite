...
function WithdrawFunds() {
  ...
  const { handleWithdraw } = useBenkikoApiDepositWithdrawOperations();
  ...
  const handleWithdrawOnClick = async () => {
    ...
    try {
      ...
      if (accessToken) {
        ...
        const handleWithdrawResponse = await handleWithdraw(
          selectedAsset,
          secretKey,
          accessToken
        );
        console.log("handleWithdrawResponse >>", handleWithdrawResponse);

        const withdrawUrl = handleWithdrawResponse.data.data.url;

        window.open(withdrawUrl, "_blank");
      }
    } catch (error) {
      console.log("Error in withdrawing asset >>>", error);
      throw error;
    }
  };
  return (
    <div className="bg-white p-5 rounded-lg shadow-md w-full max-w-xl">
      ...
      <div className="w-full flex items-center justify-between">
        ...
        <div className="mr-3 flex space-x-4">
          ...
          <Link
            to={`/settings/${userType.user}/wallet/deposit`}
            className="bg-gray-300 px-4 py-2 rounded border border-[#254E70] font-bold text-[#254E70] hover:bg-[#254E70] hover:text-white"
          >
            Deposit Funds
          </Link>
      ...
      <div className="mt-4">
        <label className="block text-gray-700">
          Select the asset type you want to Withdraw
        </label>
